...
const startManager = async (startHelper: boolean) => {
    ...
    app.post('/updateCookie', (req, res) => {
      if (req.body?.secret === managerServer.secret) {
        if (req.body?.cookie) {
          ...
          if (!req.body.cookie.includes('sc=') || !req.body.cookie.includes('PHPSESSID=')) {
  ...
  server.listen(managerServer.port, hostname, () => {
    // eslint-disable-next-line max-len
    new Logger(time() + __('managerServerStart', chalk.yellow(`${managerServer.ssl ? 'https' : 'http'}://127.0.0.1:${managerServer.port}/`)));
  ...
  async function getPid() {
    const pidRaw = await axios.get(`${webUI.ssl ? 'https' : 'http'}://127.0.0.1:${webUI.port}/run-status`)
